...
- Chaque roman a un champ « Série » libre (ex : « La Belgariade ») à remplir soi-même — Open Library ne fournit pas cette information de façon fiable au niveau œuvre. Le contrôle « Ranger par » (Grille / Série / Statut), sur la fiche dédiée d'un auteur (accessible en cliquant son nom), regroupe ensuite les romans par cette valeur, ou par statut de lecture.
- La page d'accueil affiche directement les jaquettes de chaque auteur suivi, groupées en sections — pas besoin de cliquer sur un auteur pour voir sa bibliothèque. Cliquer sur le nom d'un auteur ouvre sa fiche dédiée, avec les filtres/tri/regroupement en plus.
- Certains auteurs sont dupliqués sur Open Library sous plusieurs fiches, chacune avec une partie seulement de sa bibliographie (ex : David Eddings existe sous au moins deux identifiants différents). L'app fusionne automatiquement ces fiches par nom d'auteur : si des romans semblent manquants, cherchez à nouveau le même auteur et choisissez l'autre résultat — les nouveaux romans viennent s'ajouter au même auteur au lieu de créer une entrée séparée.
